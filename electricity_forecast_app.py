...
def forecast_lstm(model_type, df, forecast_days=7, use_enhanced=True):
    models = load_models()
    ...
    try:
        ...
        if "model" not in model_info or "scaler" not in model_info:
            st.error(f"Model {model_type} is missing required components")
            return None, None
        ...
        if len(df) < time_steps:
            st.error(f"Not enough data points. Need at least {time_steps} hours of data.")
            return None, None
    ...
    with col1:
        ...
        if hasattr(st.session_state, "show_forecast") and st.session_state.show_forecast:
            ...
            forecast_data = st.session_state.forecast_df
              # Check if forecast data exists and is valid
            if forecast_data is not None:
                ...
                st.warning("No valid forecast data available. Please try generating a forecast again.")
            st.markdown('</div>', unsafe_allow_html=True)
    
    with col2:        # Display forecast if available
        if hasattr(st.session_state, "show_forecast") and st.session_state.show_forecast:
            ...
            if forecast_df is not None:
                ...
                st.warning("No valid forecast data available to display. Please try generating a forecast again.")
        else:
